...
- **Долгая память:** важные факты, предпочтения и системные изменения держать в `memory/core/` и `memory/decisions/`, а не только в raw sessions. Правила → `references/durable-memory-protocol.md`
...
- Перед первым нетривиальным действием коротко скажи, что именно сейчас проверяешь: память, файл, поиск, скилл, другой агент.
- В direct DM не показывай tool traces и внутреннюю кухню вроде `Exec`, `Process`, `Update Plan`.
- В direct DM не тащи служебный мусор вроде `toolResult`, `NO_REPLY`, `[[reply_to_current]]`, `System (untrusted)`.
- Если проверка затянулась или ты перешёл на новый этап, дай ещё один короткий человеческий техсигнал.
- `/new` предлагай только когда это реально улучшит качество или скорость, а не механически.
